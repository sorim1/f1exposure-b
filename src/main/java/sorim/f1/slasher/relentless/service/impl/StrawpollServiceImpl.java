...
public class StrawpollServiceImpl implements StrawpollService {
    ...
    public StrawpollModelThree generateStrawpoll() {
        ...
        StrawpollPollConfig config = generateStrawpollMeta(f1calendar);
        String gpName = getRaceName(f1calendar);
        StrawpollPoll poll = StrawpollPoll.builder().poll_options(pollOptions).poll_meta(meta)
    ...
    public StrawpollPoll postStrawpoll() {
        ...
        LinkedHashMap map = (LinkedHashMap) data.getJson();
        Object pollFromDatabase = map.get("poll");
        HttpEntity entity = new HttpEntity(pollFromDatabase, headers);
